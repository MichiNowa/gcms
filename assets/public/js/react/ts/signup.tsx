...
function updateInputForm(studentid: string, firstname: string, middleinitial: string, lastname: string, email: string) {
...
function QrScanner() {
  ...
  const onResult = React.useCallback((fullname: string, studentid: string) => {
  ...
  return React.createElement(
    'div',
    { className: "tw-mx-auto tw-max-w-[320px] tw-aspect-square tw-p-4 tw-rounded-lg " + (scannedData.length > 0 ? 'tw-bg-green-300' : 'tw-bg-gray-300') },
    React.createElement(
      ReactQrScanner.Scanner,
      {
        onScan: handleScan,
        paused,
      }
